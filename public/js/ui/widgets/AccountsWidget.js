...
class AccountsWidget {
  ...
  registerEvents() {
      ...
      createAccountButton.addEventListener('click', () => {
          const modal = App.getModal('new-account'); // Открываем модальное окно для создания счета
          modal.open();
      });
  ...
  update() {
      if (!User.current()) {
          return; // Если пользователь не авторизован, выходим из метода
      }
      ...
      Account.list((err, response) => {
          ...
          this.clear(); // Очищаем список счетов перед отрисовкой новых
          response.forEach(account => {
              this.renderItem(account); // Отрисовываем каждый счет
          });
      });
  }
